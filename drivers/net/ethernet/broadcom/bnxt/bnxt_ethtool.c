...
static int bnxt_grxclsrule(struct bnxt *bp, struct ethtool_rxnfc *cmd)
{
	...
	if (fkeys->basic.n_proto == htons(ETH_P_IP)) {
		if (fkeys->basic.ip_proto == IPPROTO_ICMP ||
		    ...
		    fkeys->basic.ip_proto == 0) {
			fs->flow_type = IP_USER_FLOW;
			fs->h_u.usr_ip4_spec.ip_ver = ETH_RX_NFC_IP4;
			fs->h_u.usr_ip4_spec.proto = fkeys->basic.ip_proto;
			if (fkeys->basic.ip_proto == IPPROTO_ICMP ||
			    fkeys->basic.ip_proto == IPPROTO_RAW)
		...
		if (fkeys->basic.ip_proto == IPPROTO_ICMPV6 ||
			...
			fs->h_u.usr_ip6_spec.l4_proto = fkeys->basic.ip_proto;
			if (fkeys->basic.ip_proto == IPPROTO_ICMPV6 ||
			    fkeys->basic.ip_proto == IPPROTO_RAW)
				    ...
				    struct ethtool_rxnfc *cmd)
{
	...
	case IP_USER_FLOW: {
		...
		fkeys->basic.ip_proto = ip_mask->proto ? ip_spec->proto : 0;
	...
	case IPV6_USER_FLOW: {
		...
		fkeys->basic.ip_proto = ip_mask->l4_proto ? ip_spec->l4_proto : 0;
